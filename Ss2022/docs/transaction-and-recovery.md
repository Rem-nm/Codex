...
常规配置变更默认保留最近 10 个备份，也可从菜单立即创建手动快照。快照先写入同一文件系统的隐藏准备目录，完成状态语义检查和全树 `fsync` 后才通过目录重命名发布。列出、恢复和自动清理只接受目录名、元数据、必要文件与节点/流量/历史语义都完整的 Ss2022 快照；不完整目录或管理员放在 `backups/` 下的其他内容会保留。schema 5 快照完整保存 VLESS UUID、Reality Private/Public Key、Short ID、SNI、握手目标和 Flow，HY2 密码、跳跃范围，以及 TUIC UUID/Password/固定参数，并保存 HY2/TUIC TLS SNI、证书 pin 和证书目录、订阅设置和 token；恢复时不重新生成，因此原客户端身份和订阅地址继续可用，派生订阅输出从恢复后的节点库重建。删除节点时是否保留流量历史由用户确认；保留时把当前快照写入 `traffic-history.json` 的 `deleted_nodes`，选择不保留则同时清除该 Node ID 的已结算周期，并删除其派生证书目录。
