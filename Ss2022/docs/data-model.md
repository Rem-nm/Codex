...
schema 2 使用 `protocol` 判别联合：`shadowsocks` 必须且只能包含 method/password 协议字段；`vless` 必须且只能包含 UUID、固定 Flow 和完整 Reality 字段。公共状态字段只维护一份。Node ID、名称和端口全局唯一；VLESS UUID、Reality private/public key 和 Short ID 也分别保持节点间唯一，防止导入或恢复时悄悄共用身份。名称、端口只用于展示和监听；Node ID 是永久身份。允许名称重命名、端口重分配和协议凭据重生成，历史始终通过 Node ID 关联。第一版不支持在 SS2022 与 VLESS 之间转换协议，候选事务也会拒绝既有 Node ID 改变协议。
...
旧 `schema_version: 1` 只表示 SS2022 节点。升级时先备份节点、流量、历史、manager 状态和当前 sing-box 配置，再原子写入 schema 2，并为每个旧节点增加 `protocol: "shadowsocks"`。除这个判别字段与顶层 schema 版本外不改变旧节点内容。
...
所有持久字节字段和 `quota_bytes` 都限制在 JSON/IEEE-754 可精确表达的 `0..9007199254740991`。SS2022 与 VLESS 共用 manager 的全局配额策略，默认计费值为 `current_download_bytes`；端口层 `current_upload_bytes` 仍显示和归档，但不用于自动停用。若管理员启用既有全局上传计费选项，则全部协议统一按上传加下载触发，不能为单个协议单独切换。下载计数也可能包含 TCP 握手回复等未认证响应，因此该策略只降低未认证流量的影响，不提供认证级计费保证。
...
- `bandwidth-plan.json`：boot ID、接口、实际启用地址族，以及每个启用节点/方向的 action kind、index、cookie、端口、速率和实际传输列表。SS2022 action 使用 `["tcp","udp"]`，VLESS action 使用 `["tcp"]`；启动校验会把它与节点协议和接口状态交叉核对。旧计划缺少传输字段时仅按旧 SS2022 的 TCP/UDP 语义读取。
